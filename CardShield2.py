...
def predict_fraud(data):
    """
    Rule-based prediction: Amount > 5000 or V1 < -3 is fraud
    """
    if data["Amount"] > 5000 or data.get("V1", 0) < -3:
        return "Fraud 🚨"
    else:
        return "Legit ✅"
...
with tab1:
    ...
    V1 = col3.number_input("V1", value=0.0, format="%.2f")

    if st.button("Run Prediction"):
        transaction = {"Time": Time, "Amount": Amount, "V1": V1}
...
with tab2:
    ...
    st.markdown("Upload CSV with columns: Time, Amount, V1 (other V2..V28 optional).")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Run Batch Prediction"):
            df["prediction"] = df.apply(lambda row: predict_fraud(row), axis=1)
...
st.markdown("---")
st.caption("FalconShield — Dummy Fraud Detector • Rule-based version for demo/prototyping.")
